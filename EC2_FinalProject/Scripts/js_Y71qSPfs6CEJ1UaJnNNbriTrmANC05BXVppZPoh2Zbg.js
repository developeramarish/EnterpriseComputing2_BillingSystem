...
(function ($, document, window, viewport) {
    //JS to allow linking to tabs within iframed pages for the Cayman Site.
var iFrameReloaded = false;
var iFrameResized;

$(document).ready(function(){
	if(!document.getElementById("main-iframe")){
		postProcess();
	}
});
   window.postProcess = function(){
		...
		if(tab != ""){
			var items = (jQuery)(".nav-tabs > li");
			(jQuery).each(items, function(i){
				var href = (jQuery)((jQuery)(items[i]).find('a')).attr("href");
				if(href.indexOf(tab) != -1 ){
					var place = tab;
					(jQuery)("#myCarousel").carousel(parseInt(place));
					(jQuery)('.carousel').carousel('pause');
					(jQuery)((jQuery)(items[i])).addClass("active");
					var tabPanes = (jQuery)(".tab-pane");
					(jQuery).each(tabPanes, function(i){
						var paneId = (jQuery)(tabPanes[i]).attr("id");
						if(paneId == place){
							(jQuery)(tabPanes[i]).addClass("active");
						}else{
							(jQuery)(tabPanes[i]).removeClass("active");
						}
					});
				}else{
					(jQuery)((jQuery)(items[i])).removeClass("active");
				}
			});
		}else if(document.getElementById('main-iframe') && !iFrameResized ){ //resize the iframe to fit to contents
		...
		if (iframe) {
			...
			if (iframeWin.document.body) {
				iframe.height = iframeWin.document.documentElement.scrollHeight || iframeWin.document.body.scrollHeight;
						 ...
						 iframe.width= "100%";
				jQuery("#main-iframe").attr("scrolling", "no");
			}
		}
	}
...
(function ($, document, window, viewport) {
    ...
    $(window).resize(
        viewport.changed(function(){
            if( viewport.is('>=md') ) {
                ...
                clearFooter();
                $(".region-sidebar-first").removeClass("sb-fixed");
            }
    ...
    function fixedSidebar() {
        var headerHeight = $('header#navbar').outerHeight() + 51;
        var sbFirst = $(".region-sidebar-first");
        var browser = $(window).height();
        var doc = $( document ).height();
        var main = $('#block-system-main').outerHeight();

        $(window).scroll(function () {
            var scroll = $(window).scrollTop();

            //console.log('browser: ' + browser + ' doc: ' + doc + ' main: ' + main + ' scroll: ' + scroll + ' sidebar: ' + sbFirst.height() + ' header: ' + headerHeight );

            if (browser > sbFirst.height() && sbFirst.height() < main && viewport.is('>=md')) {
                if (scroll > headerHeight) {
                    sbFirst.addClass("sb-fixed");
                } else {
                    sbFirst.removeClass("sb-fixed");
                }
            }
        });
    }
    ...
    function collapseSidebar() {
        var slideout = new Slideout({
            ...
            'menu': document.getElementById('sb1'),
            'padding': 256,
            'tolerance': 70
        });

        document.querySelector('.sbToggle').addEventListener('click', function() {
            slideout.toggle();
        });

        slideout.on('beforeopen', function() {
            document.querySelector('#sbNavIcon').classList.remove('fa-chevron-right');
            document.querySelector('#sbNavIcon').classList.add('fa-chevron-left');
        });

        slideout.on('beforeclose', function() {
            document.querySelector('#sbNavIcon').classList.remove('fa-chevron-left');
            document.querySelector('#sbNavIcon').classList.add('fa-chevron-right');
        });
    }
    ...
    function setFooter() {
        ...
        $(footer).css('height', footerHeight).addClass('bottom');
        $('body').css('margin-bottom', footerHeight);
    }

    function clearFooter() {
        $('body').removeAttr('style', '');
        $('#footer').removeAttr('style', '').removeClass('bottom');
    }
    ...
    // Search Button
/*
    $(document).on('show.bs.dropdown', '#search-block-group', function () {
        $('#search-trigger .fa').removeClass('fa-search').addClass('fa-times-circle');
    });

    $(document).on('hide.bs.dropdown', '#search-block-group', function () {
        $('#search-trigger .fa').removeClass('fa-times-circle').addClass('fa-search');
    })
*/
    $('.grid').masonry({
